# Empujar una Imagen Docker a Amazon ECR

Este ejercicio práctico consiste en contenedorizar una aplicación web estática y subir la imagen al registro privado de **Amazon Elastic Container Registry (ECR)**.

## Prerrequisitos

- Docker instalado y en funcionamiento.
- [AWS CLI](https://aws.amazon.com) configurado con tus credenciales.
- Permisos IAM para administrar repositorios en ECR.

## Archivos del Proyecto

### 1. Aplicación Web (`index.html`)
Un sitio sencillo con estilos CSS embebidos para dar la bienvenida a AWS.

### 2. Dockerfile
Utilizamos una imagen base de **Nginx Alpine** por su eficiencia y ligereza:

dockerfile
```
FROM nginx:alpine
COPY index.html /usr/share/nginx/html
EXPOSE 80
```

## 3. Pasos para el Despliegue

### 1. Crear el Repositorio en ECR
1. Ir a la consola de Amazon ECR
2. Hacer clic en **Create Repository**
3. Nombre del repositorio **simple-web-app**
4. Mantener la configuración de visibilidad como **Private**

![Previsualizacion](ECR_1.png)

### 4. Ejecución de comandos

1. Tener instalado la librería **aws configure** en el sistema operativo o el entorno virtual de VSCode
2. Tener un usuario IAM la cual contiene el permiso o política **AmazonEC2ContainerRegistryFullAccess**
3. Ejecutar el comando en la terminal de la carpeta -> aws configure 
4. Ingresar de forma secuencial la Access Key y Secret Key
5. Dirigirse al repostorio privado **simple-web-app**
6. Hacer clic en **Ver comandos de envío**
7. Ejecutar los siguientes codigos de manera secuencial en la terminal

![Comandos](ECR_2.png)


8. Actualizar las imágenes en **simple-web-app**


Listo ya estaría desplegada una imagen Docker en Amazon ECR

## Resultado Esperado

![Resultado Esperado](ECR_3.png)
